...
export function FloatingInfo() {
  ...
  const positionClass = isWorkspace 
    ? "bottom-6 left-6" 
    : "top-24 right-4 sm:top-auto sm:bottom-6 sm:right-6";
  const popupClass = isWorkspace
    ? "bottom-14 left-0 mb-2"
    : "top-14 sm:top-auto sm:bottom-14 right-0 mt-2 sm:mt-0 sm:mb-2";

  return (
    <div className={`fixed z-[100] ${positionClass}`}>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute w-72 rounded-xl border border-border-default bg-surface-default p-4 shadow-2xl ${popupClass}`}
          >
