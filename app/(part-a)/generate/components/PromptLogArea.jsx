...
export function PromptLogArea({ sessionLogs, isGenerating, currentPrompt, aspectRatio, imageCount, error, onRetry, onTryPrompt, onPreviewImage }) {
  ...
  return (
    <>
      <div 
        ref={containerRef}
        className="relative flex flex-1 flex-col overflow-y-auto overflow-x-hidden bg-background px-4 pt-[80px] pb-40 md:px-8 md:pb-52 custom-scrollbar"
      >
              ...
              {log.urls.length === 1 ? (
                <div className="flex justify-center w-full">
                  <div
                    ...
                    className="relative w-full max-w-[400px] sm:max-w-[500px] overflow-hidden rounded-xl border border-border-subtle shadow-lg group bg-surface-default cursor-zoom-in"
                    onClick={() => onPreviewImage && onPreviewImage(log.urls[0])}
                  >
                    <Image
                      src={log.urls[0]}
                      alt={`${log.prompt}`}
                ...
                <div className={`grid gap-4 w-full ${log.urls.length === 2 ? 'grid-cols-2 max-w-2xl mx-auto' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4'}`}>
                  {log.urls.map((url, i) => (
                    <div
                      key={i}
                      style={{ aspectRatio: log.aspectRatio.replace(':', '/') }}
                      className="relative w-full overflow-hidden rounded-xl border border-border-subtle shadow-lg group bg-surface-default cursor-zoom-in"
              ...
              {Number(imageCount) === 1 ? (
                <div className="flex justify-center w-full">
                  <div
                    ...
                    className="relative w-full max-w-[400px] sm:max-w-[500px] overflow-hidden rounded-xl border border-border-subtle shadow-lg bg-surface-default"
                  >
                    <Skeleton className="absolute inset-0 h-full w-full rounded-none" />
                  </div>
                </div>
              ) : (
                <div className={`grid gap-4 w-full ${Number(imageCount) === 2 ? 'grid-cols-2 max-w-2xl mx-auto' : 'grid-cols-2 sm:grid-cols-3 lg:grid-cols-4'}`}>
